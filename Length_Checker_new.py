...
window = tk.Tk()
window.title("Length Checker V1.2")
...
def create_tab_content(default_net_names):
    ...
    def process_tab(file_path, tab_name, brd_name):
        ...
        for i in range(4):
            ...
            target_third_line = length_fields[i].get()
            match_list = []
            no_match_list = []
    ...
    def output_report(all_no_match_lists, all_match_lists, tab_name, brd_name):
        ...
        for i in range(4):
            report_content += f"\n{net_name_fields[i].get()}:{length_fields[i].get()}\n\n"
    ...
    for i in range(4):
        ...
        length_label = Label(frame, text="設定線長")
        length_label.grid(row=1, column=i * 2, padx=5, pady=5, sticky='w')

        length_field = Entry(frame)
