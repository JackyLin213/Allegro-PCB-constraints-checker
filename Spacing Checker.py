...
class TabContent(Frame):
    def __init__(self, parent, default_net_name, default_net_name2, default_net_name3):
        ...
        for i in range(3):
            ...
            line_to_via_label.grid(row=1, column=i*4+2, padx=5, pady=5, sticky='w')
            line_to_via_area = scrolledtext.ScrolledText(self, height=10, width=40)
            line_to_via_area.grid(row=2, column=i*4+2, columnspan=2, padx=5, pady=5, sticky="nsew")
    ...
    def process_tab(self, file_path):
        for i in range(3):
            ...
            try:
                self.process_file(file_path, target_net_name, line_to_line_list, line_to_via_list, line_to_shape_list)
    ...
    def process_file(self, file_path, target_net_name, line_to_line_list, line_to_via_list, line_to_shape_list):
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            for line in file:
                ...
                if len(parts) >= 5 and parts[0] == "Net":
